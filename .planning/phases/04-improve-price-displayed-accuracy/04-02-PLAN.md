...
must_haves:
  ...
  artifacts:
    ...
    - path: "Toplist Final/toplist-main/toplist-main/src/components/PriceBreakdown.jsx"
      provides: "Detailed price breakdown component"
...
Add this method after the `getPropertyAvailability` method (around line 66).
  </action>
...
<task type="auto">
  <name>Task 2: Create PriceBreakdown component</name>
  <files>Toplist Final/toplist-main/toplist-main/src/components/PriceBreakdown.jsx</files>
  <action>
Create a new React component that displays the full price breakdown:
 ...
 */
const PriceBreakdown = ({ price, loading }) => {
  if (loading) {
    return (
...
The component matches the existing dark theme with Tailwind classes.
  </action>
  ...
  <done>PriceBreakdown.jsx exists with loading state, full breakdown display, and Airbnb disclaimer</done>
</task>
...
2. Add state for price estimate after existing state declarations (around line 32):
   ```jsx
   const [priceEstimate, setPriceEstimate] = useState(null);
   const [loadingPrice, setLoadingPrice] = useState(false);
   ```

3. Add useEffect to fetch price when dates change (after line 139, near other useEffects):
   ```jsx
...
4. Replace the booking sidebar price section (lines ~659-667, the div with "Base Price" and "Total per night"):
   ...
   <div className="mb-4">
     {priceEstimate || loadingPrice ? (
       <PriceBreakdown price={priceEstimate} loading={loadingPrice} />
     ) : (
       <div className="flex justify-between items-center">
...
This shows the basic nightly rate before dates are selected, then switches to full breakdown once dates are chosen.
  </action>
  <verify>
1. Check import: `grep -n "import PriceBreakdown" "Toplist Final/toplist-main/toplist-main/src/pages/PropertyDetails.jsx"`
2. Check state: `grep -n "priceEstimate" "Toplist Final/toplist-main/toplist-main/src/pages/PropertyDetails.jsx"`
...
4. Build check: `cd "Toplist Final/toplist-main/toplist-main" && npm run build` (if available)
  ...
  <done>PropertyDetails.jsx fetches price estimate on date change, shows PriceBreakdown when dates selected, falls back to base rate otherwise</done>
</task>
...
2. PriceBreakdown.jsx exists and is valid JSX
3. PropertyDetails.jsx imports and uses PriceBreakdown
