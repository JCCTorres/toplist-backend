...
class BookervilleController extends Controller
{
     ...
     * IDs are strings to preserve precision for large Airbnb IDs
     */
    ...
    public function getPropertyDetails(Request $request, string $propertyId): JsonResponse
    {
        ...
        try {
            ...
            // Get airbnb_id from database or fallback to embedded mapping
            $airbnbId = null;
            $localProp = Property::where('property_id', $propertyId)->first();
            if ($localProp && $localProp->airbnb_id) {
                $airbnbId = (string) $localProp->airbnb_id;
            } elseif (isset(self::AIRBNB_ID_MAPPING[$propertyId])) {
                // Fallback to embedded mapping if not in database
                $airbnbId = self::AIRBNB_ID_MAPPING[$propertyId];
            }

            // Merge airbnb_id into response if available
            if ($airbnbId) {
    ...
    public function getHomeCards(Request $request): JsonResponse
    {
        try {
            ...
            // Reference to mapping for use in closure
            $airbnbMapping = self::AIRBNB_ID_MAPPING;

            // Formatar dados para o frontend
            $formatCard = function ($property) use ($airbnbMapping) {
                ...
                // Get airbnb_id from property or fallback to mapping
                $airbnbId = $property->airbnb_id ? (string) $property->airbnb_id : null;
                if (!$airbnbId && isset($airbnbMapping[$property->property_id])) {
                    $airbnbId = $airbnbMapping[$property->property_id];
                }
